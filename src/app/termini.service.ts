...
export class TerminiService {
  ...
  constructor(private serviceStanovi: StanoviService, private  http: HttpClient, private authService: AuthService) {
  ...
  public dodajTerminUBazu(id: string, idStana: string, grad: string, adresa: string, broj: string, brojTelefonaVlasnika: string, emailVlasnika: string, datum: string, vreme: string,
    ...
    return this.authService.token.pipe(
      take(1),
      switchMap((token) => {
        return this.http.post<{name: string}>('https://mr-app-d15ba-default-rtdb.europe-west1.firebasedatabase.app/termin.json?auth=' + token,
  ...
  public ucitajTerminIzBaze() {
    return this.authService.token.pipe(
      take(1),
      switchMap((token) => {
        return this.http.get<{ [key: string]: TerminPodaci }>('https://mr-app-d15ba-default-rtdb.europe-west1.firebasedatabase.app/termin.json?auth=' + token)
  ...
  public izmeniTermin(id: string, idStana: string, grad: string, adresa: string, broj: string, brojTelefonaVlasnika: string, emailVlasnika: string, datum: string, vreme: string,
    ...
    return this.authService.token.pipe(
      ...
      switchMap((token) => {
        ...
        ('https://mr-app-d15ba-default-rtdb.europe-west1.firebasedatabase.app/termin/' + id + '.json?auth=' + token,
          {id,idStana,grad,adresa,broj,brojTelefonaVlasnika,emailVlasnika,datum,vreme,emailKupca,status})
          .pipe(map((resData) =>{
            this.termini.getValue().push({
              id,
